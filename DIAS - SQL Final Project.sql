...
INSERT INTO AWARDS VALUES (269, 4, 39, "Primetime Emmy Awards", 2018, "Winner");
 ...
 -- No. Of times Actors have been nominated but did not win the awards
...
-- This query calculates the no. Of seasons for each show which have been completed to date
SELECT Show_Title AS "Show Title", (Show_End_Year - Show_Start_Year) AS "No. of Seasons"
FROM TV_Shows
WHERE Show_End_Year is not null;

-- Displaying Shows which have not been completed yet
SELECT Show_Title AS "Show Title", Show_Start_Year AS "Show Start Year"
FROM TV_Shows
WHERE isnull(Show_End_Year);

-- The query below shows the no. Of movies each actor performed in descending order
...
-- Top 3 "crime" TV Shows based on votes  
...
-- Number of movies by genre in the database 
select movie_genre, count(movie_genre) AS 'Count'  
from Movie  
group by movie_genre  
order by count(movie_genre) DESC;

-- Number of awards won per actor while also displaying their birth year 
...
-- The query below will display directors, the movies they directed, and their star 
-- ratings that are above or equal to 9.0. Based on the output we discover that there is only
-- one director who appears twice on this list. When this output is compared to the data, 
-- there are several directors who appear more than once, but only one appears twice with
-- the filters created in the following query.
...
-- The following query will display information of TV shows that have a rating of > 8.5 and avg review length of > 250
--  with an average review length of above 250. We discover that there are only three TV
-- shows that have a high rating with a higher average review length.
...
-- Top TV Shows that have a high star rating, are intended for mature audiences, have a 
-- credible number of votes, has concluded, and has more than a 3 year running period
SELECT Show_Title, Show_Plot_Description, Show_Votes, (Show_End_Year - Show_Start_Year) AS "Show Duration in years" 
FROM TV_SHOWS 
WHERE Show_Star_Rating > 9.0 
AND Show_Votes > 350000 
AND Show_Age_Restriction = "TV-MA" 
AND (Show_End_Year - Show_Start_Year) > 3 
GROUP BY Show_Title, Show_Plot_Description, Show_Votes, (Show_End_Year - Show_Start_Year) 
ORDER BY Show_Votes DESC;
